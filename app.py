...
# Helper for attorney performance
def get_attorney_performance(df, metric='invoice_total', top_n=10):
    ...
    if metric == 'invoice_total':
        ...
        # Total collected
        attorney_perf = df.groupby('Originator')['Payments_Applied_Against_Invoice_in_USD'].sum().reset_index()
        attorney_perf.columns = ['Attorney', 'Value']
        attorney_perf['Value'] = attorney_perf['Value'].abs()  # Convert to positive for display
        attorney_perf['Metric'] = 'Total Collected'
    elif metric == 'delay':
        # Average payment delay
        attorney_perf = df[df['Days between Invoice date and last payment date'] != 'Unpaid']
    ...
    if not attorney_perf.empty:
        if metric == 'delay':  # For delay, smaller is better
            top = attorney_perf.nsmallest(top_n, 'Value')
            bottom = attorney_perf.nlargest(top_n, 'Value')
        else:  # For invoiced and collected, higher is better
            top = attorney_perf.nlargest(top_n, 'Value')
            bottom = attorney_perf.nsmallest(top_n, 'Value')
...
def main():
    ...
    with st.expander("🔧 Filters", expanded=True):
        ...
        if 'Originator' in df.columns:
            with col3:
                attorneys = ['All'] + sorted(df['Originator'].unique().tolist())
                selected_attorney = st.selectbox("Attorney", attorneys)
    ...
    with tab1:
        ...
        if 'Invoice_Date' in df_filtered.columns and not df_filtered['Invoice_Date'].empty:
            ...
            # Create bar chart for YoY comparison
            fig = go.Figure()
    ...
    with tab2:
        ...
        if 'Invoice_Date' in df_filtered.columns and not df_filtered['Invoice_Date'].empty:
            ...
            st.warning("Date information is not available for paid vs outstanding analysis.")
        ...
        # For this section, we would need target data which is not in the current dataset
        # This is a placeholder that could be enhanced with actual targets
    ...
    with tab3:
        ...
        if 'Accounting Entity' in df_filtered.columns:
            ...
            offices = ['All'] + sorted(df_filtered['Accounting Entity'].unique().tolist())
    ...
    with tab4:
        ...
        with col1:
            status_options = ['All'] + sorted(df_filtered['Invoice Status'].unique().tolist()) if 'Invoice Status' in df_filtered.columns else ['All']
        ...
        with col3:
            max_amount = st.number_input("Max Amount", min_value=0.0, value=float(df_filtered['Invoice_Total_in_USD'].max()) if 'Invoice_Total_in_USD' in df_filtered.columns else 1000000.0)
        ...
        if search_term:
            ...
            if 'Client' in filtered_invoices.columns:
                search_mask |= filtered_invoices['Client'].astype(str).str.contains(search_term, case=False, na=False)
            
            if 'Matter' in filtered_invoices.columns:
                search_mask |= filtered_invoices['Matter'].astype(str).str.contains(search_term, case=False, na=False)
            
            if 'Invoice_Number' in filtered_invoices.columns:
                search_mask |= filtered_invoices['Invoice_Number'].astype(str).str.contains(search_term, case=False, na=False)
            
            filtered_invoices = filtered_invoices[search_mask]
        
        # Highlight overdue invoices
        if 'Last payment date' in filtered_invoices.columns and 'Invoice_Date' in filtered_invoices.columns and 'Invoice_Balance_Due_in_USD' in filtered_invoices.columns:
    ...
    with tab5:
        ...
        if 'Client' in df_filtered.columns and 'Days between Invoice date and last payment date' in df_filtered.columns:
            ...
            if not payment_delay_df.empty:
                ...
                st.warning("Payment delay data is not available.")
        else:
        ...
        if 'Invoice_Date' in df_filtered.columns and 'Invoice_Balance_Due_in_USD' in df_filtered.columns:
        ...
        if 'Invoice_Total_in_USD' in df_filtered.columns and 'Days between Invoice date and last payment date' in df_filtered.columns:
            # Prepare data
            ...
            if not scatter_df.empty:
                ...
                st.warning("Payment speed data is not available.")
        else:
